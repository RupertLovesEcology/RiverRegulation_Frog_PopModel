...
## randomly assign probability of wet or dry for wither natural or regulated flow scenarios over a range of sill heights
...
## set source NOTE in Rversion 3.6.2 must be set to the U:drive on the flinders network and not to the local documents folder
...
outPut <- array(data = 0, dim = 64 * 80 * 18)
...
#outPut remains safely in state. We pass the values to outPut2 to shuffle and process. 
outPut2 <- array(data = 0, dim = 64 * 80 * 18)
dim(outPut2) <- c(8,8,18,4,20)
rownames(outPut2) <- c("DDD","DDW","DWD","DWW","WDD","WDW","WWD","WWW")
colnames(outPut2) <- c("DDD","DDW","DWD","DWW","WDD","WDW","WWD","WWW")
...
## Store the individual population changes for each wet/Dry pattern popChanges
## Note third dimension is 1 - Starting Adult, 2 - finishing adult, 3 - change in adult, 
# 4 - if adults was > 0 at start and ==0 at end store a 1(extinction this run), 5 - Starting all, 6 - Finishing all, 7 - Change in all (in %), 
# 8 - if pop goes extinct 1 otherwise 0
popChanges <- array(data = NA, dim = 512000000)
dim(popChanges) <- c(8,8,10,800000)

## iterate projection (currently using 100 runs for speed)
iter <- 10000
itdiv <- iter/10
...
######################################################
## Process the WetDry likelyhood for each of 18 sill heights and regulations 
## 7N, 7R, 7.5N, 7.5R - 11N, 11R
## Store the % in transitionLikelyhood
######################################################
...
# collect every natural and guage height data for winter and spring in an array
...
#write.csv(WetDry,"c:/Workspace/WetDry.csv")

## Create an array to store the likelyhood of transitioning from wet to dry/dry to wet
...
#Likelyhood of change for each of 2 states (18 columns)
for (r in 1:ncol(WetDry)) {
  ...
  #calculate, assign the %change and clear the counters
...
## sillFlow tracks the likelyhood of transition under sill height 7 m - 11 m and regulated vs natural as 1:18  as found in transitionLikelyhood 
for (sillFlow in 1:18) {
  cat("starting sillFlow ", sillFlow, " which is ", NameList[sillFlow],"\n")
  
  ## The Outermost Loop: we will cycle through population sizes (1,2,3,4/S,M,L,XL)
  for (size in 1:4) {
    
    ## The Second Loop: iterate the process iter times
    for (e in 1:iter) {
      ...
      for (i in 1:t) {
        ...
        #This bit is broken ATM ignoring graphing for now
        #update r.stoch 
        ...
        while  (J <= nchar(WDTracker)) {
          ...
          if (end + 1 - J > 1) {
            ...
            if  (sum(n.mat[2:6,J - 1,size]) == 0) {
              ## Sometimes we cant use this run for adult extinction because the starting pop is 0 we will keep count of this in 10
...
########  THE FOLLOWING CODE IS PRIMARILY FOR VISUALISING EARLY OUTPUTS
...
## Already done don't rerun use saved images'
## Make a plot 1
...
ggplot(data = dfGraph2 , aes(x = Dry)) +
  ...
  scale_linetype_manual("", breaks = c("Small", "Medium", "Large","Very.Large"), values = c("dotted", "dotdash", "dashed","solid")) +
  xlab(" ") 
 # scale_y_continuous("Temperatura (C)", limits = c(0,100))  
